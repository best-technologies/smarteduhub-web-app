...
type ParsedTeacher = TeacherFormData & {
  row: number;
  errors: string[];
};

interface TeacherUploadSectionProps {
  onTeachersUploaded: (teachers: TeacherFormData[]) => void;
...
export default function TeacherUploadSection({
  onTeachersUploaded,
  existingTeachers,
}: TeacherUploadSectionProps) {
      ...
      reader.onload = (e) => {
        try {
          ...
          const parsed: ParsedTeacher[] = (
            ...
            // Check for duplicate email in existing teachers
            if (
              email &&
              ...
              errors.push("Email already exists in added teachers");
            }
  ...
  const handleConfirmUpload = () => {
    const validTeachers = parsedTeachers.filter(
      (teacher) => teacher.errors.length === 0
    );

    const uniqueTeachers = validTeachers.filter((teacher, index, array) => {
      return array.findIndex((t) => t.email === teacher.email) === index;
    });

    const teachersToAdd = uniqueTeachers.map((teacher) => ({
    ...
    onTeachersUploaded(teachersToAdd);
        ...
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Preview Upload ({parsedTeachers.length} teachers found)
            </DialogTitle>
          </DialogHeader>
          <div className="py-4">
            <div className="mb-4">
              <div className="flex items-center gap-4 text-sm">
                <span className="text-green-600">
                  ✓ Valid:{" "}
                  {parsedTeachers.filter((t) => t.errors.length === 0).length}
                </span>
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {parsedTeachers.map((teacher, index) => (
                <div
                  key={`${teacher.email}-${teacher.row}-${index}`}
                    ...
                    <div className="mt-2">
                      <ul className="text-xs text-red-600">
                        {teacher.errors.map((error, i) => (
                          <li key={`${teacher.row}-error-${i}`}>• {error}</li>
                        ))}
                      </ul>
                    </div>
                  )}
            ...
            <div className="flex gap-3 mt-6">
              ...
              >
                Add {parsedTeachers.filter((t) => t.errors.length === 0).length}{" "}
                Valid Teachers
              </Button>
            </div>
          </div>
